...
public class ProblemaC {
    ...
    public static No unflagNo(String s, No novoNo)
    {
		...
		}else if(novoNo.matricula.compareToIgnoreCase(s) < 0)
		{
			...
			tree.leftHeight = getHeight(tree.left);
		 	...
		 	if(Math.abs(tree.leftHeight - tree.rightHeight) == 2)
		 		tree = avl(tree);
		 	else
		 	{
			   tree.resetHeight();
    ...
    public static void changeColors(No novoNo){
    	No rightChild = novoNo.right;
        if(rightChild != null){
        	if(rightChild.colorblack == false && novoNo.colorblack == false){
				novoNo.colorblack = true;
			}

			if(rightChild.colorblack == true && novoNo.colorblack == true){
				if(novoNo != tree){
					rightChild.colorblack = false;
				}
			}

			if(rightChild.right != null && rightChild.right.colorblack == true){
				if(rightChild != tree){
					novoNo.colorblack = false;
				}
			}
			else if(rightChild.right != null && rightChild.right.colorblack == false){
				rightChild.colorblack = true;
			}

			if(rightChild.left != null && rightChild.left.colorblack == true){
				if(rightChild != tree){
					rightChild.colorblack = false;
				}
			}
			else if(rightChild.left != null && rightChild.left.colorblack == false){
				rightChild.colorblack = true;
			}
        }
        if(novoNo.left != null){
            changeColors(novoNo.left);
        }

        if(novoNo.right != null){
            changeColors(novoNo.right);
        }
    }
...
class No
{
      ...
      No(String matricula, String estado, boolean cor) {
